...
def chk_mkdir(newdir, nreturn=False):
    ...
    if nreturn == True:
        return newdir
def get_frame(path_str): 
    return int(path_str.split('/')[-1].split('_')[-1].split('.')[0])
...
def make_pdbs_from_traj(outdir, trajfile, pdb, dock_name):
    ...
    u = mda.Universe(pdb, trajfile)
    
    protein = u.select_atoms("protein")
    
    for ts in u.trajectory:
        frame_num = ts.frame
        protein.write(f'{trajpdbs}/{dock_name}_{frame_num}.pdb')
    
    return lsdir(trajpdbs), u

###########################################################################################
"""

def _make_pdbs_from_traj(outdir, trajfile, pdb, dock_name): 
...
def run_diffdock(outdir, git_repo, trajfile, pdb, ligfile): 
    ...
    print(f"Docking on {protein_traj.trajectory.n_frames} now. Hang tight! :) ")

    # Actually running diffdock: 
    sp.run(['python3', f'{git_repo}/inference.py', '--config', f'{git_repo}/default_inference_args.yaml', '--protein_ligand_csv', run_csv, '--out_dir', outdir, '--samples_per_complex', '15', '--model_dir', f'{git_repo}/workdir/paper_score_model', '--confidence_model_dir', f'{git_repo}/workdir/paper_confidence_model']) 
...
def make_traj(outdir, protein_u, ligpdbs, success_ind, dock_name):
    
    print(f" number of starting protein frames: {success_ind.shape[0]}")
    ligand_pdbs_paths = lsdir(ligpdbs)
    valid_ligand_paths = []
    valid_indices = []
    
    for i, lig_path in enumerate(ligand_pdbs_paths):
        try:
            tmp = mda.Universe(lig_path)
            valid_ligand_paths.append(lig_path)
            if i < len(success_ind): 
                valid_indices.append(success_ind[i])
        except Exception as e:
            print(f"Failed to load: {lig_path}, Error: {e}")
            
    print(f'new protein frame amount: {len(valid_indices)}')
    print(f'the amount of ligand pdbs we have now: {len(valid_ligand_paths)}')

    if len(valid_ligand_paths) == 0:
        print("No valid ligands to merge.")
        return

    ref_ligand_u = mda.Universe(valid_ligand_paths[0])
    protein_atoms = protein_u.select_atoms("protein")
    ligand_atoms = ref_ligand_u.atoms
    
    combined_u = mda.Merge(protein_atoms, ligand_atoms)
    
    outtrajs = chk_mkdir(f'{outdir}/trajoutfiles', nreturn=True)
    out_xtc = f'{outtrajs}/{dock_name}_out.xtc'
    out_pdb = f'{outtrajs}/{dock_name}_out.pdb'
    
    protein_u.trajectory[valid_indices[0]] 
    combined_u.atoms[:len(protein_atoms)].positions = protein_atoms.positions
    first_lig_u = mda.Universe(valid_ligand_paths[0])
    combined_u.atoms[len(protein_atoms):].positions = first_lig_u.atoms.positions
    combined_u.atoms.write(out_pdb)

    with mda.Writer(out_xtc, combined_u.atoms.n_atoms) as W:
        for i, frame_idx in enumerate(valid_indices):
            
            protein_u.trajectory[frame_idx]
            combined_u.atoms[:len(protein_atoms)].positions = protein_atoms.positions
            
            current_lig_path = valid_ligand_paths[i]
            current_lig_u = mda.Universe(current_lig_path)
            combined_u.atoms[len(protein_atoms):].positions = current_lig_u.atoms.positions
            
            W.write(combined_u.atoms)
            
    print(f"Merged trajectory saved to {out_xtc}")

####################################################################################
"""
def _make_traj(outdir, protein_traj, ligpdbs, success_ind, dock_name): 
    """ Makes a trajectory given a list of ligand pdbs, indices of the failed frames, and the original trajectory. """
# trying to load each pdb with md traj. if we can't, take it out of our ligand pdb list and successful indices. 

    print(f" number of starting protein frames: {success_ind.shape[0]}") 
    ligand_pdbs = lsdir(ligpdbs)
    extra_failed = []
    count = 0
    leng = len(ligand_pdbs)
    while count != leng: 
        count = 0
        leng = len(ligand_pdbs)
        try:    
            for i, lig in enumerate(ligand_pdbs): 
                current_lig = lig
                md.load(lig)
                count = count + 1
        except: 
            print(current_lig)
            extra_failed.append(get_frame(current_lig))
            ligand_pdbs.remove(current_lig)
            leng = len(ligand_pdbs)
    print(f'failed idx : {extra_failed}') 
    suc_idx = list(success_ind)
    for i in extra_failed: 
        suc_idx.remove(i)

    print(f'new protein frame amount: {success_ind.shape[0]}') 
    print(f'the amount of ligand pdbs we have now: {len(ligand_pdbs)}') 

    # Slicing the original protein traj to match 
    sliced_protein_traj = protein_traj[np.array(suc_idx)]
     
    # loading a ligand trajectory from the pdbs and stacking with the sliced protein trajectory 
    ligtraj = md.load(ligand_pdbs)
